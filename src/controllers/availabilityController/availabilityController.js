...
const { generateStartTimes, addOneHour, isWeekend, parseISODateOnly } = require("../../utils/slots");
...
exports.getVenueAvailability = async (req, res) => {
  try {
    ...
    if (!day) return res.status(400).json({ message: "Invalid date format (YYYY-MM-DD)" });
    ...
    const [bookings, cartItems] = await Promise.all([
      ...
      prisma.cartItem.findMany({
        where: { venueId, date: day, expiresAt: { gt: now } },
        select: { court: true, startTime: true, userId: true },
      }),
    ]);

    const bookedSet = new Set(bookings.map((b) => `${b.court}|${b.startTime}`));
    const inCartMap = new Map(cartItems.map((c) => [`${c.court}|${c.startTime}`, c.userId]));

    const starts = generateStartTimes({ openHour: OPEN_HOUR, closeHour: CLOSE_HOUR });
    ...
    const slots = starts.map((start) => {
      ...
      const cells = COURTS_FIXED.map((court) => {
        ...
        if (booked) return { court, state: "BOOKED", price };
        if (inCartUserId) {
          return {
            court,
